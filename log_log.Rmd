...
wtp_lm <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own + log(gdp_prop) + log(years_fishing) + log(fishing_org_members) + education + factor(country), data = bind_log_final)
...
## education: a more educated person will likely want to pay more for this technology.. they will see more benefit in it. 
...
## "By controlling for country in the original equation, we are estimating the relationship between wtp and our independent variables having adjusted for systematic differences in willingness to pay and our independent variables between Indonesia and Mexico."

```

```{r}
# calculate exact percent change in WTP for our categorical variables 
coef_lm <- as.data.frame(wtp_lm$coefficients) %>%
  tibble::rownames_to_column("Beta") %>%
  mutate(on = exp(wtp_lm$coefficients) - 1 , off = exp(-wtp_lm$coefficients) - 1)

## TODO ## Figure out the interpretation of the continuous variables 
```
...
The unconditional expected geometric mean of our willingness to pay variable is $1.67596. 



**Predictors that are categorical (Approximate Interpretation):**
...
**Predictors that are categorical (Exact Interpretation):**
I will only show the proof for one of our categorical variables, as it could get redundant otherwise. 

SOS: 

$$ "Turning \ SOS \ on" \ \frac{(PWTP_{SOS = 1} - PWTP_{SOS = 0})}{PWTP_{SOS = 0}} = \frac{PWTP_{SOS = 1}}{PWTP_{SOS = 0}} - 1 = e^{\beta_1} - 1 = e^{0.1718} - 1 =  0.18744 $$
 ...
 $$ "Turning \ SOS \ off" \  \frac{(PWTP_{SOS = 0} - PWTP_{SOS = 1})}{PWTP_{SOS = 1}} = \frac{PWTP_{SOS = 0}}{PWTP_{SOS = 1}} - 1 = e^{\beta_1} - 1 = e^{-0.1718} - 1 =  -0.1578 $$
...
Predictors that are continuous:
