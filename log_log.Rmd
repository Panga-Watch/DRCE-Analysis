...
mylogit <- glm(choice ~ wtp + sos + info + own + education + fishtech + rank_one, data = bind_log_final, family = "binomial")

summary(mylogit)
mylogit_rows <- c(names(mylogit$coefficients))

coef_binom <- data.frame(mylogit_rows, coefs = mylogit$coefficients) %>%
  mutate(wtp_model = coefs/mylogit$coefficients[2])
```
...
bind_interval_mex <- bind_log_final %>% 
  filter(country == "MEX") %>%
  mutate(wtp_upper = ifelse(wtp == -7.75, -5.17, 
...
AIC(int_tech, int, int_1, int_2, int_3, int_4, int_5, int_6, int_7, int_8, int_9, int_10, int_11, int_12, int_13, int_15)
```
