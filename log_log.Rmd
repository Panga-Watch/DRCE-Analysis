...
Predict on our data with int_16 and see if predicted values fall into interval range: GRAPHS
```{r}
...
int_predictions %>%
  filter(country == "IND") %>%
  ggplot(aes(x = bid_intervals_mean, y = wtp_predict)) +
  geom_point() +
  ...
  labs(title = "Overall Model : Indonesia Prediction Results") +
...
int_predictions %>%
  ...
  geom_errorbarh(aes(xmin =  bid_intervals_mean - (interval_range/2), xmax = bid_intervals_mean + (interval_range/2)), height = 0) +
  labs(title = "Overall Model : Mexico Prediction Results") +
  geom_abline(intercept = 0, slope = 1) 


## now need to do parameretize model with indonesia data and predict on mexico and vice versa and make these graphs
```
...
indo_model_mexi_df_predict %>%
  ...
  scale_y_continuous(breaks = c(-5.17, -3.62, -2.07, 0, 2.07, 3.62, 5.17, 7.75, 13.33), limits = c(-5.17, 13.33)) +
  geom_errorbarh(aes(xmin =  bid_intervals_mean - (interval_range/2), xmax = bid_intervals_mean + (interval_range/2), height = 0)) +
  labs(title = "Indo Model Predicted on Mexican Data") +
  geom_abline(intercept = 0, slope = 1)
...
mexi_model_indo_df_predict %>%
  ...
  scale_y_continuous(breaks = c(-5.35, -3.56, -2.14, 0, 2.14, 3.56, 5.35, 7.12, 11.02), limits = c(-5.35, 11.02)) +
  geom_errorbarh(aes(xmin =  bid_intervals_mean - (interval_range/2), xmax = bid_intervals_mean + (interval_range/2), height = 0)) +
  labs(title = "Mexican Model Predicted on Indo Data") +
  geom_abline(intercept = 0, slope = 1)
```
