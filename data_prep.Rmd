...
pooled_data_fina_fixl <- pooled_data_final_fix %>%
  mutate(sos = as.factor(sos), info = as.factor(info), own = as.factor(own))
...
ce_total <- arrange(ce_total, survey_id)
...
```{r}
## issue occurring when package_nopackage == 0... need to fix this so it is correct.

# ce_total_spread_package <- ce_total %>%
#   select(survey_id, package, RES) %>%
#   group_by(survey_id) %>%
#   summarise_each(funs(paste(., collapse = "-"))) %>%
#   mutate(RES = ifelse(RES == "0-1-1", "0-0-1", RES)) %>%
#   mutate(RES = ifelse(RES == "1-0-1", "0-0-1", RES)) %>%
#   mutate(package_join = package,
#          RES_join = RES) %>%
#   separate(package, c("package_offer_1", "package_offer_2", "package_offer_3")) %>%
#   #separate(RES, c("RES_1", "RES_2", "RES_3")) %>%
#   gather(var, package, package_offer_1:package_offer_3) %>%
#   select(-RES)
#   #gather(var_RES, RES, RES_1:RES_3) %>%
#   #arrange(survey_id, var_RES)
# 
# ce_total_spread_RES <- ce_total %>%
#   select(survey_id, package, RES) %>%
#   group_by(survey_id) %>%
#   summarise_each(funs(paste(., collapse = "-"))) %>%
#   mutate(RES = ifelse(RES == "0-1-1", "0-0-1", RES)) %>%
...
#   #gather(var, package, package_offer_1:package_offer_3)
# 
# ce_total_spread_package_111 <- ce_total_spread_package %>%
#   select(-var, -package_join, -RES_join)
# ce_total_fix <- inner_join(ce_total_spread_package, ce_total_spread_RES, by = c("survey_id", "package_join", "RES_join"))  
# 
# ce_total_fix_1 <- head(ce_total_fix, 615)
# ce_total_fix_1 <- ce_total_fix_1 %>%
#   select(-package, -var, -var_RES, -package_join, -RES_join)
# 
# ce_total_fix_final <- inner_join(ce_total_fix_1, ce_total_spread_package_111)
```
