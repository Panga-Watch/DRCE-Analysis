...
actual_choose <- pooled_mlogit_data_final %>%
  filter(choice == "TRUE") %>%
  group_by(survey_version, choice, package, info, sos, own) %>%
  summarise(actual_percent = 100*sum(choice)/211,
            count = sum(choice)) ## shows actual proportion of accepted packages.

sum(actual_choose$actual_percent)
sum(actual_choose$count)


```

Attempt to add characteristics into the model 
 - Having issues with this, because it is now giving us estimates based on the two options we gave them.  - 2:intercept is the beta value associated with each 2nd option offered I believe. 
```{r}
m_pooled_char <- mlogit(choice ~ sos + info + own | community + age + income + education, pooled_mlogit_data_final) ## is this correctly adding attributes to model?? It is comparing to each alternative offerred....
...
Try with just base R lm. Not sure that it makes sense to do it this way. 
```{r}
## Try with the base R lm like JM and JC suggested... Would this be a correct application of our data, just a simple binomial logistic regression? This does not take into account any of the trade offs between packages offered, simply looks at the attributes overall.

survey_logit <- glm(choice ~ sos +  info + own , family = "binomial", data = pooled_mlogit_data_final)

summary(survey_logit)

fitted(survey_logit)

fitted <- data.frame(probs = fitted(survey_logit)) # this shows all of the predicted probabilities for each of the choices offered apparently... 

new <- pooled_mlogit_data %>%
  select(package, STR.y, info, sos, own)

model_preds <- cbind(new, fitted) 
...
standard_mlogit <- mlogit(choice ~sos + info + own , standard_mlogit_data_final, reflevel = 'C')

actual_choose_standard <- standard_mlogit_data_final %>%
  #filter(choice == "TRUE") %>%
  group_by(survey_version, choice, package, info, sos, own) %>%
  summarise(actual_percent = 100*sum(choice)/211,
            count = sum(choice)) ## look at survey version 1... package 18 (1,1,2) was never chosen, and package 2 (1,1,3) was always chosen

# 2 (1,1,3) vs 1 (0, 1, 2)
# 2 (1,1,3) vs 8 (0, 1, 4)
# 8 and 1 never chosen when compared to 2 
# Tells us that in these cases respodents likely valued ownership of data over anonymity 

sum(actual_choose_standard$actual_percent)
sum(actual_choose_standard$count)
...
exp(m_pooled$coefficients[1]*0 + m_pooled$coefficients[2] + m_pooled$coefficients[3] +  m_pooled$coefficients[4]*0 +  m_pooled$coefficients[5]*0)/(1 + exp( m_pooled$coefficients[1]*0 + m_pooled$coefficients[2] + m_pooled$coefficients[3] +  m_pooled$coefficients[4]*0 +  m_pooled$coefficients[5]*0)) ## theoretically this should be the probability for sos =0, info = 1, and own = 1... first survey done... doesn't match the outcome of fitted()... but does match output of linear predictor
