...
```{r}
variables_log<-variables %>% 
  select(survey_id, country, community, years_fishing, age, boat_length_m, fishing_organization, punishment_severity, opinion_a, opinion_b, opinion_c, opinion_d, opinion_e, opinion_f, opinion_g, opinion_h, opinion_i, trust_a, trust_b, trust_c, trust_d, trust_e, trust_f, trust_g, trust_h, trust_i, trust_j, trust_k, trust_l, catch_trends, income_fishing, income_expenses) %>% 
...
wtp_lm_15 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + catch_trends, data = bind_log_final) 

summary(wtp_lm_15)

# Opinion C: PERCEPTION OF FISHING STOCK STATUS "In general, the fisheries in this region are doing well". 5: Strongly agree, implies a .38% reduction in payment.

wtp_lm_16 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + factor(opinion_c), data = bind_log_final) 

summary(wtp_lm_16)
# case_when with this one. if 4 or 5, = healthy. if 1 or 2 = unhealthy. if 3 = neutral.

# Opinion F: PERCEPTION OF INDUSTRIAL OUTSIDE FISHING "Outside industrial fishers are always entering our fishing grounds/concession". 

wtp_lm_17 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + factor(opinion_f), data = bind_log_final) 
# wtp increases as they agree more that industrial fishers are entering their grounds. But the highest value is number 3 (neutral)

summary(wtp_lm_17)

 # Trust in Fishing association leaders (TRUST C): 37.c. Please indicate how much you trust the following groups: c. Fishing association leaders

wtp_lm_18 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + factor(trust_c), data = bind_log_final) 

summary(wtp_lm_18)

# The more they trust in fishing association leaders, the less they are willing to pay. Does it make sense??

# Trust in POLICE 36.e
wtp_lm_19 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + factor(trust_e), data = bind_log_final) 

summary(wtp_lm_19)

# The more they trust in police, the less they are wiling to pay

wtp_lm_20 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + factor(trust_k), data = bind_log_final) 

summary(wtp_lm_20)

# % income from fishing

wtp_lm_21 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + income_fishing, data = bind_log_final) 

summary(wtp_lm_21)

# % income expenses in fishing

wtp_lm_22 <- lm(log(wtp + 1 - min(wtp)) ~ sos + info + own  + income_expenses, data = bind_log_final) 
