...
survey <- read_csv("raw/master_survey_resp.csv", string) %>% 
  left_join(bind_df) %>% 
...
q4<-survey %>% 
  ...
  select(starts_with("gear"), survey_id, wtp) %>% 
  na.omit() 
