...
try_psc<-psc_predict %>% 
  ...
  group_by(package, int_16_wtp) %>% 
  tally() %>% 
 # ungroup() %>% 
  mutate(prop=n/sum(n)) %>% 
  arrange(desc(int_16_wtp)) %>% 
  mutate(prop_sum = cumsum(prop)) 


ggplot(try_psc, aes(x= int_16_wtp)) +
...
graph_data_pepela <- Pepela_all_predictions %>%
  ...
  ggplot(graph_data_pepela, aes(x = y, y = perc, color = factor(sos))) + 
    ...
    labs(x = "Cost to Fisher ($ per month)", y = "Percentage of Population to Opt In", title = "Pepela: GFW Realistic Technology") +
    geom_vline(xintercept = c(-1.88, -0.61), linetype = "dotted", color = "darkgreen") +
     #geom_vline(xintercept = c(3.15, 4.41), linetype = "dotted", color = "red") +
        scale_x_continuous(breaks = c(-2, -1.88, -0.61, 0, 0.86, 2 ,4)) +
