...
best_tech <- data.frame(sos = c(1), info = c(0), own = c(1))

best_tech_psc <- rep(best_tech, 1800)
...
psc <- data.frame(best_tech_psc,
                  fish_tech = sample(rep(fishtech, round(nrow(best_tech_psc)*psc_prob_fishtech))),
                  education = sample(rep(education, round(nrow(best_tech_psc)*psc_prob_edu))),
                  rank_one = "iuu"
                  )
