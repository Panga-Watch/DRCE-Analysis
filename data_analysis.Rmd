...
master_survey <- read.csv("raw/master_survey_resp.csv", stringsAsFactors = TRUE) %>%
  select(-start, -end, -survey_type, -survey_version, -interviewer, -comment, -text_vms_adoption, -fishery_problem_one, -fishery_problem_two, -fishery_problem_three, -fishery_solution_one, -fishery_solution_two, -fishery_solution_three, -photo)
```
...
standard_clogit$coefficients[1]*((exp(( standard_clogit$coefficients[2]*1 + standard_clogit$coefficients[3]*0 + standard_clogit$coefficients[4]*1 + standard_clogit$coefficients[5]*0 + standard_clogit$coefficients[6]*0)))/
  (1 + (exp(( standard_clogit$coefficients[2]*1 + standard_clogit$coefficients[3]*0 + standard_clogit$coefficients[4]*1 + standard_clogit$coefficients[5]*0 + standard_clogit$coefficients[6]*0)))))
```
...
standard_predictions <- predict(standard_clogit, newdata = standard_opt_out, type = "expected", se.fit = TRUE)
...
standard_predictions_data <- standard_predictions_data %>%
  mutate(`probability (%)` = round(fit/(fit+ 1),4)*100,
         level_se = se.fit/(se.fit + 1)) %>%
  select(-fit, -se.fit, -STR, -level_se) %>%
  arrange(-`probability (%)`) %>%
  select(info, own, sos, `probability (%)`) %>%
  mutate(info = case_when(
    info == 0 ~ "Anon",
    info == 1 ~ "Non-Anon"
  ), 
